...
import { useAddUpdateCliftonAboutMutation, useGetCliftonAboutQuery } from "../../../redux/api/aboutPageApi/cliftonAboutApi";




const CliftonAboutMng = () => {
  ...
  const [addUpdateCliftonAbout, { isSuccess, isLoading }] = useAddUpdateCliftonAboutMutation();
  const [imageFile, setImageFile] = useState<any>();
  const [formCliftonAboutAdd] = Form.useForm();
  const [fileList, setFileList] = useState<UploadFile[]>([]);
  useEffect(() => {
    if (isSuccess) {
      formCliftonAboutAdd.resetFields();
      setFileList([]);
      setImageFile(null)
      formCliftonAboutAdd.setFieldsValue(cliftonAboutData)
    }
  }, [cliftonAboutData, formCliftonAboutAdd, isSuccess])
  useEffect(() => {
    if (isSuccess) {
  ...
  const onFinish = async (values: any) => {
    try {
      await formCliftonAboutAdd.validateFields();
      console.log(values);
      ...
      await addUpdateCliftonAbout(formData)
    ...
    <div className="mt-12 px-12 admin-management">
      ...
      <div className="flex flex-col gap-y-6 mt-3">
        <Spin spinning={isLoading || isLoadingData}>
          ...
          {cliftonAboutData &&
            <Form layout="vertical" form={formCliftonAboutAdd}
              name="add-about-brand"
              onFinish={onFinish}
              className="form-add-student-assessment"
              initialValues={cliftonAboutData}
            >
...
export default CliftonAboutMng