...
const LessonCardTwoMng: React.FC<{ lessons: any }> = (props) => {
    const [addLesson, { isSuccess, isLoading }] = useAddUpdateLessonMutation();
    const [imageFile, setImageFile] = useState<any>();
    const [formLessonsAddTwo] = Form.useForm();
    const [fileList, setFileList] = useState<UploadFile[]>([]);
    useEffect(() => {
        formLessonsAddTwo.setFieldsValue(props.lessons)
    }, [props.lessons])
    useEffect(() => {
        if (isSuccess) {
            formLessonsAddTwo.resetFields()
            setFileList([]);
            setImageFile('')
            formLessonsAddTwo.setFieldsValue(props.lessons)
        }
    }, [formLessonsAddTwo, isSuccess])
    useEffect(() => {
        if (isSuccess) {
    ...
    const onFinish = async (values: any) => {
        try {
            await formLessonsAddTwo.validateFields();
            const formData = new FormData();
            formData.append("title_two_en", values?.title_two_en);
            formData.append("title_two_ar", values?.title_two_ar);
            formData.append("content_two_ar", values?.content_two_ar);
            formData.append("content_two_en", values?.content_two_en);
            formData.append("icon_two", imageFile);
            ...
            <Spin spinning={isLoading}>
                {props.lessons &&
                    <div className="flex flex-col gap-y-6 ">
                        <Form layout="vertical" form={formLessonsAddTwo}
                            name="add-lesson-1"
                            onFinish={onFinish}
                            ...
                            <div className="grid grid-row-2 gap-y-6">
                                <div className="grid grid-cols-2 gap-x-6">
                                    <div className="flex flex-col ">
                                        ...
                                        <img src={props.lessons?.icon_two}
                                         alt="" className="w-[50px] h-[50px] mb-6" />

                                    </div>
                                    <div className="flex flex-col">

                                        <Form.Item label="Description English" name="content_two_en"
                                            rules={RulesName({ name: `Description English`, countChar: 1500 })}

                                        >
                                            <Input.TextArea />
                                        </Form.Item>
                                        <Form.Item label="Description Arabic" name="content_two_ar"
...
export default LessonCardTwoMng