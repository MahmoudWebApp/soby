...
const LessonCardOneMng: React.FC<{ lessons: any }> = (props) => {
    const [addLesson, { isSuccess, isLoading }] = useAddUpdateLessonMutation();
    const [imageFile, setImageFile] = useState<any>();
    const [formLessonsAddOne] = Form.useForm();
    const [fileList, setFileList] = useState<UploadFile[]>([]);
    useEffect(() => {
        formLessonsAddOne.setFieldsValue(props.lessons)
    }, [props.lessons])
    useEffect(() => {
        if (isSuccess) {
            formLessonsAddOne.resetFields()
            setFileList([]);
            setImageFile('')
            formLessonsAddOne.setFieldsValue(props.lessons)
        }
    }, [formLessonsAddOne, isSuccess])
    useEffect(() => {
        if (isSuccess) {
    ...
    const onFinish = async (values: any) => {
        try {
            await formLessonsAddOne.validateFields();
            const formData = new FormData();
            formData.append("title_one_en", values?.title_one_en);
            formData.append("title_one_ar", values?.title_one_ar);
            formData.append("content_one_ar", values?.content_one_ar);
            formData.append("content_one_en", values?.content_one_en);
            formData.append("icon_one", imageFile);
            ...
            <Spin spinning={isLoading}>
                {props.lessons &&
                    <div className="flex flex-col gap-y-6 ">
                        <Form layout="vertical" form={formLessonsAddOne}
                            name="add-lesson-1"
                            onFinish={onFinish}
                        ...
                        >
                            <div className="grid grid-row-2 gap-y-6">
                                <div className="grid grid-cols-2 gap-x-6">
                                    ...
                                    <img src={props.lessons?.icon_one}  alt=""  className="w-[50px] h-[50px] mb-6"/>

                                    </div>
                                    <div className="flex flex-col">

                                        <Form.Item label="Description English" name="content_one_en"
                                            rules={RulesName({ name: `Description English`, countChar: 1500 })}

                                        >
                                            <Input.TextArea />
                                        </Form.Item>
                                        <Form.Item label="Description Arabic" name="content_one_ar"
...
export default LessonCardOneMng