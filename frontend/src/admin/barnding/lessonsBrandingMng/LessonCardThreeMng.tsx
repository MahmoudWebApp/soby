...
const LessonCardThreeMng: React.FC<{ lessons: any }> = (props) => {
    const [addLesson, { isSuccess, isLoading }] = useAddUpdateLessonMutation();
    const [imageFile, setImageFile] = useState<any>();
    const [formLessonsAddThree] = Form.useForm();
    const [fileList, setFileList] = useState<UploadFile[]>([]);
    useEffect(() => {
        formLessonsAddThree.setFieldsValue(props.lessons)
    }, [props.lessons])
    useEffect(() => {
        if (isSuccess) {
            formLessonsAddThree.resetFields()
            setFileList([]);
            setImageFile('')
            formLessonsAddThree.setFieldsValue(props.lessons)
        }
    }, [formLessonsAddThree, isSuccess])
    useEffect(() => {
        if (isSuccess) {
    ...
    const onFinish = async (values: any) => {
        try {
            await formLessonsAddThree.validateFields();
            const formData = new FormData();
            formData.append("title_three_en", values?.title_three_en);
            formData.append("title_three_ar", values?.title_three_ar);
            formData.append("content_three_ar", values?.content_three_ar);
            formData.append("content_three_en", values?.content_three_en);
            formData.append("icon_three", imageFile);
            ...
            <Spin spinning={isLoading}>
                {props.lessons &&
                    <div className="flex flex-col gap-y-6 ">
                        <Form layout="vertical" form={formLessonsAddThree}
                            name="add-lesson-1"
                            onFinish={onFinish}
                            ...
                            <div className="grid grid-row-2 gap-y-6">
                                <div className="grid grid-cols-2 gap-x-6">
                                    <div className="flex flex-col ">
                                        ...
                                        <img src={props.lessons?.icon_three}
                                         alt="" className="w-[50px] h-[50px] mb-6" />

                                    </div>
                                    <div className="flex flex-col">

                                        <Form.Item label="Description English" name="content_three_en"
                                            rules={RulesName({ name: `Description English`, countChar: 1500 })}

                                        >
                                            <Input.TextArea />
                                        </Form.Item>
                                        <Form.Item label="Description Arabic" name="content_three_ar"
...
export default LessonCardThreeMng